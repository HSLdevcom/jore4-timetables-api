...
COMMAND=${1}
# Shift the arguments so that we can refer to them later with "$@".
shift
...
download_docker_bundle() {
  ...
  local gh_path="/repos/${repo_owner}/${repo_name}"
  ...
  # failed.
  if [[ $? -ne 0 ]]; then
    echo "Error: Querying GH API with the commit ref '${commit_ref}' failed." >&2
    exit 1
  fi
  ...
  gh api "${gh_path}/zipball/${commit_sha}" > "$zip_file" \
  ...
  # Create a release version file containing the SHA digest of referenced commit.
