...
class TimetablesDataInserterRunner(
    ...
    fun insertDataset(datasetInputFile: String) {
        LOGGER.info("Inserting dataset from file: $datasetInputFile")
        val datasetFilePath =
            Paths.get(datasetInputFile).toAbsolutePath().toString()

        val datasetJson = File(datasetFilePath).readText()

        runTimetablesDataInserter(datasetJson)
        LOGGER.info("Inserting dataset finished.")
    }
    ...
    fun runTimetablesDataInserter(datasetJson: String): String {
        ...
        var process = ProcessBuilder(*command.split(" ").toTypedArray())
